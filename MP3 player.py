...
image_icon = PhotoImage(file="saurav img.png")
root.iconphoto(False, image_icon)




img = Image.open('Alan Walker.gif')
frameCnt = img.n_frames

frames = []
for i in range(frameCnt):
    img.seek(i)
    photo = ImageTk.PhotoImage(img.copy().resize((485, 400)).convert('RGBA'))
    frames.append(photo)
...
def update(ind):
    ...
    label.image = frame  # keep a reference to the image
